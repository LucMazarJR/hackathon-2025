 ...
 */
export const authLogAdmin = async (req, res, next) => {
    const authHeader = req.headers["authorization"];
    if (!authHeader)
        return res.status(401).json({ msg: "Não autorizado!" });
    const token = authHeader.split(" ")[1];
    const secretADM = process.env.TOKEN_ADM;
    try {
 ...
 * @returns {Promise<void>}
 */
export const authLogDoctor = async (req, res, next) => {
    const authHeader = req.headers["authorization"];
    if (!authHeader)
        return res.status(401).json({ msg: "Não autorizado!" });
    const token = authHeader.split(" ")[1];
 ...
 * @returns {Promise<void>}
 */
export const authLogUser = async (req, res, next) => {
    const authHeader = req.headers["authorization"];
    if (!authHeader)
        return res.status(401).json({ msg: "Não autorizado!" });
    const token = authHeader.split(" ")[1];
