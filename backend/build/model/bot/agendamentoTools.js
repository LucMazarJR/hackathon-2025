...
import { buscarMedicosPorEspecialidade, buscarMedicosPorCidade, verificarDisponibilidade, criarAgendamento, obterMedicoPorId, listarEspecialidades, listarCidades } from "../../database/bot/agendamento.js";
// Função para verificar se uma data está dentro do próximo mês
const isWithinNextMonth = (dateString) => {
...
export const buscarMedicosDisponiveis = tool({
    ...
    execute: async ({ especialidade, cidade }) => {
        let medicos = [];
        ...
        return medicosComDisponibilidade.map(medico => `ID: ${medico.id} - ${medico.nome} (${medico.especialidade}) - ${medico.cidade}\n` +
...
export const verificarDisponibilidadeEspecifica = tool({
    ...
    execute: async ({ medicoId, data, horario }) => {
...
export const realizarAgendamento = tool({
    ...
    execute: async ({ medicoId, paciente, data, horario }) => {
        const medico = obterMedicoPorId(medicoId);
        if (!medico) {
