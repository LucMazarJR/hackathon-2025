 ...
 * }
 */
export let messageBotController = async (message, sessionId = "default") => {
    try {
 ...
 */
export let documentUploadController = async (file, message = "", sessionId = "default") => {
    try {
        // Extrai texto do documento
        const extractedText = await TextExtractionService.extractText(file.path);
        // Formata o contexto do documento
        const documentContext = TextExtractionService.formatDocumentContext(extractedText, file.originalname);
