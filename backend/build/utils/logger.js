...
export const connectServer = async (PORT) => {
    app.listen(PORT, () => {
    ...
    // DOCTOR
    app.post(routerLog.loginDoctor, Middleware.authLogDoctor, async (req, res) => { });
    // USER
    ...
    app.post(routerBot.bot, async (req, res) => {
        ...
        const { id } = req.params;
        try {
            let [status, messageBot] = await botController.messageBotController(message, id);
    ...
    app.post(routerBot.botDocument, upload.single('document'), async (req, res) => {
        ...
        try {
            let [status, messageBot] = await botController.documentUploadController(file, message, id);
            return res.status(status).json({ message: messageBot });
        }
        catch (error) {
    ...
    app.post(routeAdmin.adminContext, async (req, res) => {
        const { context } = req.body;
        try {
            let [status, message] = await adminController.adminContextController(context);
            return res.status(status).json({ message });
        }
        catch (error) {
